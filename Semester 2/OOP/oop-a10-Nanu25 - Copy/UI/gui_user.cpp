...
void GUI_USER::displayUserWLButtonHandler() {
    ...
    for(auto e : movies)
        this->ui.userListWidget->addItem(QString::fromStdString(e.toString()));
}
...
void GUI_USER::filterButtonHandler() {
    ...
    if (!filtered_movie_list.empty()) {
        ...
        QMessageBox::warning(this, "Warning", "No movies found.");
    }
}
...
void GUI_USER::showNextMovie() {
    ...
    if (!iterationStopped && current_movie_index < filtered_movie_list.size()) {
        auto movie = filtered_movie_list[current_movie_index];
        this->ui.userListWidget->addItem(QString::fromStdString(movie.toString()));
...
void GUI_USER::setTitle() {
    this->setWindowTitle("User");
}

void GUI_USER::update() {
    this->displayUserWLButtonHandler();
}
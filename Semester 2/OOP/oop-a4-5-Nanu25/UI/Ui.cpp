...
const int DISPLAY_WATCH_LIST = 3;
const int STOP = 2;

Ui::Ui(Service serv_list): service_list(serv_list) {}
...
void print_user_option() {
    ...
    std::cout << "3 for see the watchlist\n";
    std::cout << "0 to exit\n";
}

void print_movie_list(Service movie_list) {
    for(int i = 0; i < movie_list.get_list().getRepoLength(); i++) {
        std::cout << "Movie title: " << movie_list.get_list().getElement(i).getTitle() << '\n';
        std::cout << "Movie genre: " << movie_list.get_list().getElement(i).getGenre() << '\n';
        std::cout << "Movie year of release: " << movie_list.get_list().getElement(i).getYear() << '\n';
        std::cout << "Number of likes: " << movie_list.get_list().getElement(i).getLikes() << '\n';
        std::cout << "Trailer link: " << movie_list.get_list().getElement(i).getTrailer() << '\n';
        std::cout << '\n';
    }
}

void Ui::start(Service user_list) {
    Init_list(this->service_list);
    print_menu();
    ...
    while(true) {
        if(mode == ADMINISTRATOR_MODE) {
            ...
            if(option == DISPLAY_ALL_MOVIES_GIVEN_GENRE) {
                ...
                DynamicArray <TElement> filtered_movie_list = this->service_list.get_movies_given_genre(genre);

                int i = 0;
                while(i < filtered_movie_list.getLength()) {
                    std::cout << "Movie title: " << filtered_movie_list.getElem(i).getTitle() << '\n';
                    std::cout << "Movie genre: " << filtered_movie_list.getElem(i).getGenre() << '\n';
                    std::cout << "Movie year of release: " << filtered_movie_list.getElem(i).getYear() << '\n';
                    std::cout << "Number of likes: " << filtered_movie_list.getElem(i).getLikes() << '\n';
                    std::cout << "Trailer link: " << filtered_movie_list.getElem(i).getTrailer() << '\n';
                    std::cout << '\n';
                    ...
                    if(like == 1) {
                        try {
                            user_list.Add_movie(filtered_movie_list.getElem(i));
                        }catch (const std::runtime_error& e) {
                            std::cerr << "Error occurred: " << e.what() << '\n';
                        }
                    }
                    ...
                    i = (i + 1) % filtered_movie_list.getLength();
                }

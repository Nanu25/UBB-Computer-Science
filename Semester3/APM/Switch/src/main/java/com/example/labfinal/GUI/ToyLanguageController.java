...
public class ToyLanguageController {
    ...
    @FXML
    private TableView <Map.Entry<String, Value>> procedureTable;

    @FXML
    private TableColumn<Map.Entry<String, Value>, String> procedureName;

    @FXML
    private TableColumn<Map.Entry<String, Value>, Value> procedureValue;
    ...
    public void initialize() {
        ...
        procedureName.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getKey()));
        procedureValue.setCellValueFactory(cellData -> new SimpleObjectProperty<>(cellData.getValue().getValue()));
...
//                    // Update ExeStack for the selected program state
    ...
    private void updateProcTable(Map<String, Value> procTable) {
        if (procTable == null || procTable.isEmpty()) {
            procedureTable.getItems().clear();
            return;
        }
        procedureTable.getItems().clear();
        ObservableList<Map.Entry<String, Value>> procTableEntries = FXCollections.observableArrayList(procTable.entrySet());
        procedureTable.setItems(procTableEntries);
    }

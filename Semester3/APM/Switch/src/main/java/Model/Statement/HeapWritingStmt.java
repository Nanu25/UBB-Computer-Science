...
public class HeapWritingStmt implements IStmt {
    ...
    public MyIDictionary<String, IType> typecheck(MyIDictionary<String, IType> typeEnv) throws MyException {
        ...
        if (!(varType instanceof RefType)) {
            throw new MyException("HeapWritingStmt: Variable " + var_name + " is not of type RefType.");
        }
        ...
        if (!innerType.equals(expType)) {
            throw new MyException("HeapWritingStmt: Expression type " + expType + " does not match the variable's inner type " + innerType + ".");
        }

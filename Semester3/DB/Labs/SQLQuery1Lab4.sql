...
(1, 1, 10, 1)  -- Populate 10 rows in Team table
...
BEGIN
    ...
    BEGIN
        ...
        SET @teamName = CONCAT('Team_', (@i % 10) + 1); -- Example teams like Team1, Team2...
        SET @startDate = DATEADD(DAY,  10, GETDATE()); -- Start date offset by i*10 days
        SET @endDate = DATEADD(YEAR, 1, @startDate); -- End date 1 year after start date
        SET @salary = 3000 + (@i * 50); -- Example salary calculation
        SET @pid = (@i % 10) + 1; -- Example player ID (assumes max 100 players in Players table)
...
BEGIN
    ...
    BEGIN
        ...
        SET @FlightNumber = ABS(CHECKSUM(NEWID())) % 1000 + 1; -- Random flight number (1 to 1000)
        SET @DepartureDate = DATEADD(DAY, ABS(CHECKSUM(NEWID())) % 30, GETDATE()); -- Random date within 30 days
        SET @ArrivalDate = @DepartureDate; 
        SET @DepartureAirport = CHAR(65 + ABS(CHECKSUM(NEWID())) % 26) + 
                                CHAR(65 + ABS(CHECKSUM(NEWID())) % 26) +
                                CHAR(65 + ABS(CHECKSUM(NEWID())) % 26); -- Random airport code (AAA-ZZZ)

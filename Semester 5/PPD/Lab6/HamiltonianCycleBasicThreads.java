...
public class HamiltonianCycleBasicThreads {
    private final int n;
    ...
    public HamiltonianCycleBasicThreads(int n, List<Integer>[] adj) {
        this.n = n;
        this.adj = adj;
    }
    ...
    public boolean hasHamiltonianCycle(int numThreads) {
        ...
        try {
            ...
            boolean res = pool.submit(root).get();
            return res;
    ...
    private class SearchTask implements Callable<Boolean> {
        ...
        private final int parallelism;          // virtual worker budget for this subtree
        private final List<Integer> forced;     // null → normal, otherwise sequential group
        private final List<Integer> pathSoFar;  // path including current at index depth-1
        ...
        public Boolean call() throws ExecutionException, InterruptedException {
           // if (Thread.currentThread().isInterrupted() || found.get()) return found.get();
            ...
            if (workers <= 1 || k <= 1) {
                for (int next : candidates) {
                   // if (Thread.currentThread().isInterrupted() || found.get()) return found.get();
                    ...
                    pathSoFar.add(next);
                    try {
                        if (new SearchTask(next, newUsed, depth + 1, 1, null, pathSoFar).call()) {
                            found.set(true);
                            return true;
                        }
                    } finally {
                        pathSoFar.remove(pathSoFar.size() - 1); // backtrack
                    }
                }
                return found.get();
            }
            ...
            // Submit subtasks and wait for first success
            List<Future<Boolean>> futures = new ArrayList<>();
            for (Callable<Boolean> task : subtasks) {
                futures.add(pool.submit(task));
            }

            for (Future<Boolean> f : futures) {
                f.get();  // Wait for completion, but DON'T return early
            }
            return found.get();
        }
    }
...
//            }
        ...
        HamiltonianCycleBasicThreads solver = new HamiltonianCycleBasicThreads(n, adj);
            // We test 50 runs.
// For each thread count (1,2,4,8,16,32) we store the timings.
            int RUNS = 50;
            int[] threadCounts = {1, 2, 4, 8, 16, 32};

// For each thread number, we keep min/max/sum
...
// Init min values with very large numbers

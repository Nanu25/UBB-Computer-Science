...
<body>
    ...
    <div class="container">
        <h2>Actions</h2>
        <button onclick="countPeople()">Show All Creations</button>
        <button onclick="showFreeRooms()">Show Free Rooms</button>
        <button onclick="ShowMyReservations()">Show my Reservations</button>
    ...
    <div id="allProjectsSection" class="container hidden">
        <h2>Total number of guests</h2>
        <div id="allProjectsList"></div>
        <form action="countPeople.jsp" method="post">
    ...
    <!-- Free Rooms -->
    <div id="freeRoomsSection" class="container hidden">
        <h2>Free Rooms</h2>
        <div class="form-group">
            <label for="dateIn">Start Date:</label>
            <input type="date" id="dateIn" name="dateIn" required>
        </div>
        <div class="form-group">
            <label for="dateOut">End Date:</label>
            <input type="date" id="dateOut" name="dateOut" required>
        </div>
        <button onclick="loadFreeRooms()">Search Free Rooms</button>
        <div id="freeRoomsList"></div>
    </div>
    <!-- My Projects Section -->
    <div id="myReservationsSection" class="container hidden">
    ...
    <script>
        ...
        function loadMyReservations() {
            fetch('getMyReservations.jsp?userName=<%= userName %>')
                .then(response => response.text())
                .then(data => {
                    document.getElementById('myReservationsList').innerHTML = data;
                });
        }
        ...
        function showFreeRooms() {
            hideAllSections();
            document.getElementById('freeRoomsSection').classList.remove('hidden');
            //loadFreeRooms();
        }
        ...
        function hideAllSections() {
            ...
            document.getElementById('freeRoomsSection').classList.add('hidden');
            document.getElementById('myReservationsSection').classList.add('hidden');
        ...
        function loadFreeRooms() {
            ...
            if (!dateIn || !dateOut) {
                document.getElementById('freeRoomsList').innerHTML = '<div class="error">Please enter both start and end dates</div>';
                return;
            }
            
            fetch('getFreeRooms.jsp?dateIn=' + encodeURIComponent(dateIn) + '&dateOut=' + encodeURIComponent(dateOut))
                .then(response => response.text())
                .then(data => {
                    document.getElementById('freeRoomsList').innerHTML = data;
                });
        }
        ...
        function deleteMovie(movieName, userName) {
            if (confirm('Are you sure you want to delete the movie "' + movieName + " for the user " + userName + '" ?')) {
                ...
                .then(response => response.text())
                .then(result => {
                    loadFreeRooms(); // Reload the movies list
                })
                .catch(error => {

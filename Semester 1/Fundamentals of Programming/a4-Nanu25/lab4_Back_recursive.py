...
def Check_subset_has_3points_Coliniar(subset : list):
    ...
    if have3coliniar == 1:
        ...
        for i in range(1, len(subset)):
            print(number_list[subset[i]], end=" ")
        print("")

def Generate_subsets_with_at_least_3coliniar_points(step : int, subset : list, list_size : int):
    if step <= list_size:
        subset.append(0)
        for i in range(subset[step - 1] + 1, list_size):
            subset[step] = i
            if len(subset) >= 4:
                Check_subset_has_3points_Coliniar(subset)

            Generate_subsets_with_at_least_3coliniar_points(step + 1, subset, list_size)
...
subset = [-1]
Generate_subsets_with_at_least_3coliniar_points(1, subset, numbers)
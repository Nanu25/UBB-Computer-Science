...
class TextFileRepositoryBook:
    def __init__(self, file_name : str):
        self.__file_name = file_name
        self._data = {}
        self.__load_file()

    def file_format(self, book : Book) -> str:
        return str(book.id) + "," + str(book.title) + "," + str(book.author) + "\n"
    def __save_file(self):
        with open(self.__file_name, 'w') as file:
            for book in self.get_data():
    ...
    def __load_file(self):
        with open(self.__file_name, 'r') as file:
            for line in file.readlines():
                ...
                title = elements_from_file[1]
                author = elements_from_file[2]
                self.add(Book(_id, title, author))
        file.close()
